...
Routes are generated using a custom implementation of the **A\* search algorithm**, operating over a graph built from OpenStreetMap XML data (~990,000+ nodes). Spatial lookups are accelerated using a **KDTree**, which makes finding nearby nodes from an entered coordinate fast even at that scale.

Routes and saved points are stored in a **PostgreSQL database** via SQLModel, linked to user accounts so everything persists between sessions.
